...
// echo "\n starting process";
$log->write(date('Y-m-d H:i:s') . " PID: " . $process_id . " \t Start process monitor pricing rate\n");
...
//             $message = "Description: MonitorPricingRate process has been running for more than 1 minute \nProcess ID: " . $process["process_id"] . " \nTime: $date\n";
...
monitorPricingRate($notificationUrl, $api_key, $business_id);
...
// echo "\n end process";
$log->write(date('Y-m-d H:i:s') . " PID: " . $process_id . " \t End process monitor pricing rate\n\n");
...
function monitorPricingRate($notificationUrl, $api_key, $business_id)
{
    global $post, $db, $xun_numbers;

    $db->where("updated_at < date_sub(now(), interval 5 minute)");
    $db->orderBy("market_cap_rank");
    $result = $db->get('xun_cryptocurrency_rate', null, 'name,market_cap_rank,unit');

    $coinsArray = array();
    foreach ($result as $data) {
        $market_cap_rank = $data["market_cap_rank"];
        if ($data["market_cap_rank"] <= 100 && $data["market_cap_rank"] > 0) {
            array_push($coinsArray, $data["name"]);
        }
    }

    $count = count($coinsArray);

    if ($count > 0) {
        $date = date("Y-m-d H:i:s");
        // $xunNumber = array('+60192135135','+60186757884');
        $xunNumber = $xun_numbers;
        $message = "Description: (".gethostname().") Cryptocurrency that is not updated for more than 5 minutes \nCryptocurrencies: " . implode(", ", $coinsArray) . "\nTotal inactive: $count \nTime: $date\n";
        $tag = "Inactive Cryptocurrency List";
        $response = sendNewXunGnrl($notificationUrl, $api_key, $business_id, $xunNumber, $message, $tag);
        echo $response;
    }
