...
$db->where("command", "%hyperconsensus.io%", "LIKE");
$db->where("data_out", '');
$db->where("data_in", "%callback%", "LIKE");
$db->where("created_at", "2020-06-02 18:12:00", ">");
$db->where("created_at", "2020-06-02 19:42:00", "<");
$db->orderBy("id", "asc");
$res = $db->get("xun_web_services_20200602");

print_r($res);
$url = "https://member.hyperconsensus.io/scripts/reqCallback.php";
...
foreach($res as $data){
	...
	$data_in_cleaned =  str_replace('\"', '"', $data_in);
	$curl_params = json_decode($data_in_cleaned, true);

	echo "\n url $url";
	echo "\nparams";
	print_r($curl_params);

	$curl_result= $post->curl_post($url, $curl_params, 0);
	echo "\nRESULT:\n";
	print_r($curl_result);
